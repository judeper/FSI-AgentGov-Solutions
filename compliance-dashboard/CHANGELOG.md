...
- **Baseline accuracy (framework 78 → 79):** The FSI Agent Governance Framework baseline grew to 79 controls with the addition of control 2.27 (Consumption-Entitlement Governance). The shipped `sample-data/control-master.json` still contains 78 controls and does **not** include 2.27, so `README.md`, `docs/deployment-checklist.md`, and `docs/power-bi-template-spec.md` no longer describe the sample as "the full framework baseline" — they now state the 78-of-79 position and name the missing control. The sample dataset and the `PillarDimension` DATATABLE in `docs/dax-measures.md` are deliberately **unchanged**: regenerating them is a data/behavioural change requiring control 2.27's canonical metadata and re-validation, tracked separately. The `manifest.yaml` description and verification strings, and the `fsi_controlmaster` table description in `scripts/create_cd_dataverse_schema.py`, were made count-neutral so the published catalog no longer re-introduces a hardcoded control count on every framework release. (issue #205 review)
- **Command existence (second-pass):** `scripts/Get-ExchangeComplianceData.ps1` filtered Microsoft Graph `security/alerts_v2` with `$filter=category eq 'DataLossPrevention'`. The `category` property is deprecated (it holds MITRE ATT&CK kill-chain categories) and is **not** a supported `$filter` property on `alerts_v2`; the `DataLossPrevention` value is also invalid for it. Replaced with the documented, filterable `serviceSource eq 'dataLossPrevention'` enum value, and removed the unsupported `$orderby`/`$select` query options (alerts_v2 supports only `$count`, `$filter`, `$skip`, `$top`). Without this, DLP alert collection returned zero results (the request degraded silently via the catch block). Ref: Microsoft Learn `security-list-alerts_v2` and `enums-security` (serviceSource). Updated the matching caveat in `LAB-VALIDATION.md`. (lab validation, second pass)
