...
- **Baseline accuracy:** Clarified that the shipped `sample-data/control-master.json` contains the 78-control framework baseline matching every **released** framework version through v1.6.2, and that control 2.27 (Consumption-Entitlement Governance) exists only on the framework's unreleased `main` branch. The sample dataset and the `PillarDimension` DATATABLE in `docs/dax-measures.md` are deliberately unchanged — they are correct for every released framework version, and regenerating them before 2.27 ships in a release would misstate the baseline. The `manifest.yaml` description and verification strings, and the `fsi_controlmaster` table description in `scripts/create_cd_dataverse_schema.py`, were made count-neutral so the published catalog no longer re-introduces a hardcoded control count on every framework release. (issue #205 review)
