...
- **Minor (docs)**: `README.md` *Prerequisites → Licensing* listed **"Power Platform Premium"**, which is not a Microsoft SKU. The Microsoft Power Platform licensing overview lists Power Apps, Power Automate, Copilot Studio, and Power Pages as separately licensed products, and the current user license that grants premium connectors plus Dataverse storage for cloud flows is **Power Automate Premium**. Corrected the SKU name and clarified the purpose as premium-connector cloud flows. This also aligns `coi-testing` with the SKU naming already used by `action-confirmation-auditor`, `agent-365-lifecycle-governance`, `compliance-dashboard`, `finra-supervision-workflow`, and `message-center-monitor`. `README.md:122`. (drift re-verification vs Microsoft Learn, issue #336)
- **Minor (docs)**: `README.md` *Prerequisites → Licensing* listed **"Copilot Studio"** unqualified for agent API access. The Copilot Studio for Teams plan bundled with select Microsoft 365 subscriptions publishes only to Teams and cannot generate web channel security secrets, so it cannot satisfy this solution's planned Direct Line integration. Qualified the requirement as the **standalone Copilot Studio subscription** and noted the Teams-plan limitation. `README.md:124`. (drift re-verification vs Microsoft Learn, issue #336)
- **Minor (docs)**: `README.md` *Quick Start → Configure Dataverse Authentication* fenced a PowerShell statement (`$env:AZURE_MANAGED_IDENTITY_CLIENT_ID = "..."`) as `bash`. The snippet is PowerShell syntax and is not valid in bash; relabeled the fence to `powershell`. `README.md:143`. (drift re-verification, issue #336)
...
- **Docs**: Refreshed the `README.md` `Last Verified` marker to 2026-07-26 following a full re-verification of the file's Microsoft product claims against current Microsoft Learn documentation. Claims re-checked and confirmed unchanged: Direct Line remains the documented channel for connecting a Copilot Studio agent to a custom or native app; Direct Line tokens are single-conversation, expire, and must be refreshed before expiry; custom canvases must handle `application/vnd.microsoft.card.oauth` (OAuthCard) attachments when the agent requires user sign-in; a managed identity or Microsoft Entra application registered as a Dataverse application user is a supported Dataverse authentication path; and **System Administrator** remains a valid Dataverse security role name. Runner claims (`--dry-run`, `--allow-skipped`, `--category`, `--report text|json|html`, `--auth-mode client-secret` legacy fallback, exit codes 1/2/3) and the `fsi_coitestresults` status Choice values `100000000`–`100000004` were re-verified against `scripts/run_coi_tests.py` and are accurate. No regulatory or compliance wording was reviewed, added, or altered.
