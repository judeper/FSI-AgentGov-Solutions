...
- **Lab functional-verification trigger helper:** Added `scripts/New-IntakeSubmission.ps1`, a lab tool that creates a pristine `fsi_intakerequest` row in `Submitted` status (maker-supplied inputs and the T1-T6 answers only, with no router output columns) so the router flow (Flow 1) fires its "row added/modified AND status = Submitted AND blank `fsi_decisionpath`" trigger and performs the routing itself. The existing `seed-test-data.ps1` fixtures land in terminal states for reviewer-app and smoke checks and do not exercise the router; this helper supplies the missing on-demand submission. It reads the validated `scripts/seed-test-data/request-*.json` fixtures (all five scenarios), regenerates a unique `fsi_requestid` per run to avoid alternate-key collisions, supports `-DryRun` and `-RemoveAfter`, and reuses the lab's Azure CLI / `DATAVERSE_ACCESS_TOKEN` auth pattern. Verified end-to-end by a live create → status check → delete cycle on the sandbox tenant. A `-PreClassify` switch additionally stamps the validated router outputs (`fsi_pathused`, `fsi_risktier`, `fsi_zone`, `fsi_routingtopology`, `fsi_quorumrequired`, `fsi_decisionpath`, `fsi_mrmrequired`) from the fixture's `expectedClassification`/`expectedOutcome` so the vertical-slice router flow can orchestrate (status transition, self-approval override, fan-out) without re-running the already-proven classifier; verified live for both a proceed scenario (decisionPath `Express`) and a default-deny scenario (decisionPath `DefaultDeny`).
- **Lab flow-activation and connection-binding verifiers:** Added `scripts/Get-IntakeFlowState.ps1` and `scripts/Test-IntakeConnectionBinding.ps1`, two recreate-harness / functional-verification helpers. `Get-IntakeFlowState.ps1` lists the cloud flows (workflow category 5) that belong to a solution via the `solutioncomponent` table and reports each flow's `statecode` (0 = Draft/Off, 1 = Activated); with `-Activate` it PATCHes any non-activated flow to `statecode = 1` (optionally in a child-flow-first `-ActivationOrder` to avoid the ChildFlowNeverPublished race) and re-queries, and `-ExpectedCount` makes it a verification gate. `Test-IntakeConnectionBinding.ps1` reports each `fsi_cr_*` connection reference's bound/unbound state and exits non-zero unless a required set (default: the three vertical-slice references — Dataverse, Teams, Office 365) is bound. Both reuse the lab Azure CLI / `DATAVERSE_ACCESS_TOKEN` auth pattern and were verified live against the sandbox tenant (0 flows reported pre-build; the three slice references reported unbound pre-bind).
...
- **Connection references not added to the solution (provisioning):** `scripts/provision_solution_shell.ps1` created the five `fsi_cr_*` connection references via the Dataverse Web API (which places them in the Default solution) and then added them to `FSIAgentIntake` with `pac solution add-solution-component --componentType 371`. The Power Platform CLI does not recognize connection references by numeric component type — `371` (and even the platform's actual id `10137`) fails with "The provided Component Type Id ... is not known, Please provide the Component Type name", so the references silently remained in the Default solution and were invisible (and unbindable) under the `FSIAgentIntake` solution. Environment variables were unaffected because pac resolves type `380` by number. Fixed by passing the component type **name** (`-ComponentType 'connectionreference'`) and changing `Add-SolutionComponent` to accept a string type, and added an `Assert-ConnectionReferenceInSolution` post-step that throws if any expected reference is not a component of the target solution so the failure can never be silent again. Verified by a live re-run on the sandbox tenant ("Verified 4 connection reference(s) are components of solution 'FSIAgentIntake'"). Updated `docs/flow-build-prerequisites.md` (corrected binding guidance + troubleshooting) and `AGENTS.md` (gotcha writeup).
