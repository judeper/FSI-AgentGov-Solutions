...
- **Power Pages form workstream:** treat the question text, type, required-ness, and routing impact columns as the canonical maker UI contract.
- **Foundation-schema workstream:** keep the existing Express logical names as-is and add the proposed `fsi_s*` and `fsi_f*` fields without renaming the baseline columns.
...
## Proposed field families for the schema workstream

| Family | Purpose | Proposed fields |
|---|---|---|
| Existing Express fields | Current MVP baseline that must stay stable | `fsi_agentdisplayname`, `fsi_businessoutcome`, `fsi_businessjustification`, `fsi_agenttype`, `fsi_intendedaudience`, `fsi_t1initiatesfinancialtxn`, `fsi_t2customerfacing`, `fsi_t3autonomousunmonitored`, `fsi_t4handlesnpi`, `fsi_t5handlesmnpi`, `fsi_t6crossborderdata`, `fsi_dataresidencycountry`, `fsi_makerattestation` |
| Standard additions | Team-scope and moderate-risk routing | `fsi_s1audienceextension` through `fsi_s10monitoringplan` |
| Full additions | Enterprise, legal, model-risk, and resilience routing | `fsi_f1externalexposure` through `fsi_f13regulatornotificationplan` |

## Customer override boundaries

Customers should tune reviewer quorums, retention classes, sensitivity label values, NY DFS enablement, training prerequisites, and monitoring thresholds through policy/configuration rather than by renaming canonical question fields. If a customer needs more granular UI controls than a composite question provides, the downstream workstreams can split the form into sub-controls while still writing back to the same canonical question labels and decision-pack structure.
