...
**Programmatic flow build/repair via `clientdata` PATCH (verified working 2026-06-08):** a hand-built cloud flow's full definition lives in the Dataverse `workflow.clientdata` string. You **can** complete or fix a flow programmatically (not just read it): GET the `workflow` row → modify the `clientdata` JSON (`properties.definition.actions`, reusing the existing Dataverse action as the host/auth template — `authentication` = `@json(decodeBase64(triggerOutputs().headers['X-MS-APIM-Tokens']))['$ConnectionKey']`) → PATCH `statecode=0` (deactivate) → PATCH `clientdata` → PATCH `statecode=1`. **The platform fully validates on the `clientdata` PATCH** — it rejected F8 with `OpenApiOperationParameterValidationFailed` until every **ApplicationRequired** column was supplied on each `CreateRecord` (e.g., `fsi_intakedecisionlog` requires 11: name, requestid, decisionoutcome, risktier, zone, pathused, **policyversionapplied, retentionlabelapplied, retentionlabelappliedon**, decisionpackhash, decidedon). Get the required set from `EntityDefinitions(LogicalName='…')/Attributes?$select=LogicalName,RequiredLevel` (filter `ApplicationRequired`/`SystemRequired`). `UpdateRecord` does not enforce required fields. **Always back up the original `clientdata` first** (restore = exactly the prior state). After a PATCH, the maker portal must be **refreshed/reopened** to show the new version. This is supported (Dataverse Web API) — distinct from the disavowed `api.flow.microsoft.com` path.

**Programmatic flow CREATE from scratch via POST `/workflows` (verified working 2026-06-08, F1 router):** beyond repairing an existing flow, you can **create a new cloud flow** end-to-end: **POST `/workflows`** with `category=5`, `type=1`, `primaryentity="none"`, `description`, and `clientdata` (the `{properties:{connectionReferences,definition},schemaVersion}` string) plus header `MSCRM.SolutionUniqueName: FSIAgentIntake` (lands it in the solution directly, componenttype 29) → then **activate** via PATCH `statecode=1; statuscode=2`. For a **Dataverse-triggered** flow the trigger is `OpenApiConnectionWebhook` / `operationId SubscribeWebhookTrigger` (`subscriptionRequest/message`=1 Create, `/entityname`=logical singular e.g. `fsi_intakerequest`, `/scope`=4 Organization) and **both trigger and actions authenticate with `@parameters('$authentication')`** — NOT the `X-MS-APIM-Tokens` form (that is only for manual/Request-triggered flows like F8). **Activation registered the webhook and the trigger fired in <10s** on a freshly POST-created flow — so the create path (not just PATCH-update) is proven. The conn-ref (`fsi_cr_dataverse_agentintake`) must already be **bound** before activation. Pin the returned `workflowid` for stable inter-flow calls.
