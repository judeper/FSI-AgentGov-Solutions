...
<#
.SYNOPSIS
    Deploys the UASD exception approval flow to a Power Platform environment.

.DESCRIPTION
    Deploys the Unrestricted Agent Sharing Detector exception approval flow
    (Flow 3) into the target Power Platform environment using the Dataverse
...
.EXAMPLE
    .\Deploy-ExceptionApprovalFlow.ps1 -DataverseUrl "https://org.crm.dynamics.com" -SolutionPath ".\exported-exception-approval-flow.json"

.EXAMPLE
    .\Deploy-ExceptionApprovalFlow.ps1 -DataverseUrl "https://org.crm.dynamics.com" -SolutionPath ".\exported-exception-approval-flow.json" -WhatIf
...
# --- Authentication ---
Write-Host "`n[UASD Exception Approval Flow Deployment]" -ForegroundColor Cyan
...
$flowName = $flowJson.properties.displayName ?? "UASD-Exception-Approval-Workflow"
...
# --- Deploy ---
if ($PSCmdlet.ShouldProcess($flowName, "Deploy exception approval flow to $DataverseUrl")) {
    try {
        $importPayload = @{
        ...
        Write-Host "    2. Bind connection references (Dataverse, Teams, Approvals)"
        Write-Host "    3. Configure approval routing via fsi_UASD_SecurityApproverEmail (+ DataOwner, Compliance)"
        Write-Host "    4. Set exception duration limits via fsi_UASD_DefaultExceptionDays"
        Write-Host "    5. Turn on the flow"
    } catch {
