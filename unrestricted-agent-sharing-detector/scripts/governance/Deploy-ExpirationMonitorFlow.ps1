...
<#
.SYNOPSIS
    Deploys the UASD expiration monitor flow to a Power Platform environment.

.DESCRIPTION
    Deploys the Unrestricted Agent Sharing Detector expiration monitor flow
    (Flow 4) into the target Power Platform environment using the Dataverse
...
.PARAMETER SolutionPath
    Path to the expiration monitor flow JSON file

.PARAMETER TenantId
    Entra ID tenant GUID (optional, uses current Az context if omitted)

.EXAMPLE
    .\Deploy-ExpirationMonitorFlow.ps1 -DataverseUrl "https://org.crm.dynamics.com" -SolutionPath ".\exported-expiration-monitor-flow.json"

.EXAMPLE
    .\Deploy-ExpirationMonitorFlow.ps1 -DataverseUrl "https://org.crm.dynamics.com" -SolutionPath ".\exported-expiration-monitor-flow.json" -WhatIf
...
# --- Authentication ---
Write-Host "`n[UASD Expiration Monitor Flow Deployment]" -ForegroundColor Cyan
...
# --- Deploy ---
if ($PSCmdlet.ShouldProcess($flowName, "Deploy expiration monitor flow to $DataverseUrl")) {
    try {
        $importPayload = @{
        ...
        Write-Host "    3. Configure expiration check schedule (default: daily)"
        Write-Host "    4. Set fsi_UASD_ExpirationWarningDays for advance notification"
