...
.DESCRIPTION
    ...
    (uasd-remediation-apply-sharing-policy.json) into the target Power
    Platform environment using the Dataverse Web API solution import
    endpoint.
...
.EXAMPLE
    .\Deploy-RemediationFlow.ps1 -DataverseUrl "https://org.crm.dynamics.com" -SolutionPath "..\..\src\uasd-remediation-apply-sharing-policy.json"

.EXAMPLE
    .\Deploy-RemediationFlow.ps1 -DataverseUrl "https://org.crm.dynamics.com" -SolutionPath "..\..\src\uasd-remediation-apply-sharing-policy.json" -WhatIf
...
# --- Authentication ---
Write-Host "`n[UASD Remediation Flow Deployment]" -ForegroundColor Cyan
...
$flowName = $flowJson.properties.displayName ?? "UASD-Remediation-Apply-Sharing-Policy"
...
# --- Deploy ---
if ($PSCmdlet.ShouldProcess($flowName, "Deploy remediation flow to $DataverseUrl")) {
    try {
        $importPayload = @{
        ...
        Write-Host "    2. Bind connection references (Dataverse, Teams, Power Platform APIs)"
        Write-Host "    3. Configure auto-remediation via fsi_UASD_AutoRemediatePublicLink"
        Write-Host "    4. Set fsi_UASD_RemediationDryRun to true for safe initial deployment"
