...
- Fail closed when a telemetry retry sequence produces no response instead of dereferencing an uninitialized response value.
- **Incorrect Copilot Studio telemetry navigation (tech-accuracy R3 · squad audit).** `docs/pii-sanitization-guide.md` instructed operators to disable "Log sensitive Activity properties" via **Settings > Application Insights**. The documented path is **Settings > Advanced**, then the **Application Insights** section (where "Log sensitive Activity properties" lives). Corrected the navigation step; the guidance to keep the setting disabled is unchanged and correct. Source: Microsoft Learn [Capture telemetry with Application Insights](https://learn.microsoft.com/en-us/microsoft-copilot-studio/advanced-bot-framework-composer-capture-telemetry#connect-your-copilot-studio-agent-to-application-insights).
- **Graceful dependency handling:** `provision.py`, `teardown.py`, `verify_telemetry.py`, and `verify_worm.py` imported the Azure management SDKs (`azure-mgmt-*`) eagerly at module load, so even `--help` aborted with a raw `ModuleNotFoundError` traceback when the optional SDK stack was not installed. Wrapped the Azure SDK import blocks in a `try/except ImportError` guard that emits `Missing dependencies. Run: pip install -r requirements.txt` and exits with code 4, matching the repo's existing graceful-degradation pattern (`environment-lifecycle-management/scripts/register_service_principal.py`). No runtime behavior change when dependencies are present.
