...
<#
.SYNOPSIS
    Verifies SHA-256 integrity of hallucination evidence files.
...
.DESCRIPTION
    ...
    Evidence files produced by Export-HallucinationEvidence include SHA-256
    ...
    - 3.10 (Hallucination Feedback Loop) — evidence integrity
    - 2.9 (Performance Monitoring) — audit trail verification
    - 2.12 (Supervision) — examination readiness

.PARAMETER EvidenceFilePath
    Full path to the JSON evidence file to verify. File must exist.
...
.EXAMPLE
    .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath ".\evidence\ht-evidence-20260402-143022.json"
...
.EXAMPLE
    Get-ChildItem .\evidence\ht-evidence-*.json | ForEach-Object {
        .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath $_.FullName
    }

    Batch verification of all hallucination evidence files in a directory.

.EXAMPLE
    $isValid = .\Test-EvidenceIntegrity.ps1 `
        -EvidenceFilePath ".\evidence\ht-evidence-20260402-143022.json" `
        -Quiet
...
.NOTES
    ...
    - Example: "abc123...  ht-evidence-20260402-143022.json"
