#!/usr/bin/env python3
"""
Create connection references for Hallucination Tracker.

Connection references enable Power Automate flows to access Dataverse
for report persistence and Microsoft Teams for pattern alert notifications.
"""
...
CONNECTION_REFS = [
    {
        "logical_name": "fsi_cr_dataverse_hallucinationtracker",
        "display_name": "Dataverse - Hallucination Tracker",
        "connector": "shared_commondataserviceforapps",
        "description": "Dataverse connector for hallucination report persistence",
    },
    {
        "logical_name": "fsi_cr_teams_hallucinationtracker",
        "display_name": "Teams - Hallucination Tracker",
        "connector": "shared_teams",
        "description": "Microsoft Teams connector for hallucination pattern alerts",
    },
]
...
def create_connection_references(client: DataverseClient, dry_run: bool = False) -> dict:
    """
    Create connection references for Hallucination Tracker Power Automate flows.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create connection references for Hallucination Tracker",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Connection references created:
  - fsi_cr_dataverse_hallucinationtracker (Dataverse connector)
  - fsi_cr_teams_hallucinationtracker (Teams connector)
...
Examples:
  ...
  python create_ht_connection_references.py \\
      --tenant-id <tenant-id> \\
