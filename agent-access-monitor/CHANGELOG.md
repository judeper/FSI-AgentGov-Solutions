...
- **Flagged undocumented governance setting in Zone Requirements (`README.md`).** Added a NEEDS_HUMAN_REVIEW comment above the `bot-publishedBotLimitSharingMode` row in the Zone Requirements table. The canonical Microsoft Learn page ([Limit sharing — Power Platform](https://learn.microsoft.com/en-us/power-platform/admin/managed-environment-sharing-limits)) documents only three `bot-*` extended-settings keys (`bot-limitSharingMode`, `bot-authoringSharingDisabled`, `bot-maxLimitUserSharing`). `bot-publishedBotLimitSharingMode` does not appear in any reachable Learn page. The script `scripts/Get-EnvironmentAccessSettings.ps1:329` already reads this key via `Get-ExtendedSetting` with graceful null handling, so behavior is unchanged — but a live-tenant verification is required to confirm the key is a real internal/extended setting (rather than always returning null and silently masking a compliance gap). (technical accuracy review vs Microsoft Learn)
