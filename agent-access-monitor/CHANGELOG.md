...
- **Owl-mode C1 — fictitious Managed-Environment key purged.** Removed every reference to the fictitious published-bot limit sharing-mode key (a `bot-…` name that does **not** exist in the Power Platform Managed-Environment `extendedSettings` object). Deleted the corresponding Dataverse column, the baseline-JSON key, the comparison/severity logic, the evidence-export field, and all alert text. AAM now relies only on the three real keys: `bot-limitSharingMode`, `bot-authoringSharingDisabled`, and `bot-maxLimitUserSharing`.
- **Owl-mode C2 — archived MSAL.PS dependency removed** from `Start-AccessValidationRunbook.ps1`, `Invoke-AccessBaselineCapture.ps1`, and `Export-AgentAccessEvidence.ps1`. Modern OAuth is now acquired through `Get-AAMAccessToken` (device-code via `-Interactive`, or client-secret via `-ClientSecret`). Passing `-CertificateThumbprint` now throws a terminating error, because certificate-thumbprint auth required MSAL.PS.

### Changed

- **Owl-mode C3 — PowerShell 7.4 retarget** of `Start-AccessValidationRunbook.ps1`, `Invoke-AccessBaselineCapture.ps1`, and `agent-access-monitor.psd1` (`PowerShellVersion = 7.4`), because Azure Automation retired the 7.1/7.2 runtimes. Local-only tools remain at `#Requires -Version 7.0`.
- **Owl-mode C4 — "immutable audit" reframed as append-only by role design.** Removed claims of native Dataverse immutability across code and docs; the append-only audit trail is now documented as a security-role design (app user Create+Read only, deny Write/Delete) in the new [`docs/role-design-append-only.md`](docs/role-design-append-only.md).
- **Deferred Azure Automation (M1/M3).** `Start-AccessValidationRunbook.ps1` now runs standalone locally (PowerShell 7.4 via Task Scheduler) and writes to Dataverse; the daily Power Automate flow is reduced to a Recurrence trigger + Dataverse read. Azure subscription, Automation Account, premium connector, and always-on service principal are no longer required for the lab — they are an optional production upgrade.
- **Non-Managed environment guard (H1).** Environments without `extendedSettings` are now reported as `Status = NotManaged` and treated as out-of-band scope (compliant, not a violation), rather than defaulting to a bug-inducing `noLimit`.
- **Case-insensitive sharing-mode comparisons (H2).** Sharing-mode value comparisons use explicit case-insensitive operators so documented (`ExcludeSharingToSecurityGroups`) and lowercased values match.
- **Real `bot-maxLimitUserSharing` evaluated (H3).** Numeric values normalize to `Capped` (positive integer) or `Uncapped` (`-1`, `0`, blank, or unset) for per-zone comparison and severity.
- Version strings synchronized to `1.2.0` across `manifest.yaml`, `controls-covered.json`, README, `agent-access-monitor.psd1`, script `.SYNOPSIS Version:` headers, evidence export payloads, and `templates/zone-settings-baseline.json`.

### Added

- **Lab harness** under `lab/` (`Invoke-Deploy.ps1`, `Test-LabAuthReadiness.ps1`, `config.example.json`, gitignored `config.local.json`, `slice-build-runbook.md`, `README.md`) for the standalone deferred-Azure deployment model.
- **`docs/role-design-append-only.md`** describing how the append-only audit trail is achieved through security-role design (supports compliance with FINRA 4511 / SEC 17a-3 / 17a-4).

### Fixed

- **Major (lab validation):** `Get-EnvironmentAccessSettings.ps1` read agent sharing settings from `$env.Internal.governanceConfiguration.settings.extendedSettings`, which is the wrong object path. The authoritative [Power Platform "Limit sharing"](https://learn.microsoft.com/power-platform/admin/managed-environment-sharing-limits) PowerShell guidance and the script's own environment-group / managed-state reads use `$env.Internal.properties.governanceConfiguration` / `$env.Internal.properties.*`. The missing `.properties.` segment caused `bot-limitSharingMode`, `bot-authoringSharingDisabled`, and `bot-maxLimitUserSharing` to read as `null`, which `Compare-ZoneCompliance.ps1` then defaults to `noLimit`/`false` — producing systematic false Critical/High violations in Zone 2/3. Corrected both extraction sites (`Get-ExtendedSetting` and the raw-settings loop). Runtime behavior is not verifiable without a live Managed Environment; the fix is grounded in the authoritative source and internal consistency.
- **Minor (lab validation):** `templates/zone-settings-baseline.json` setting descriptions for `ExcludeSharingToSecurityGroups` were reversed — they described sharing as "restricted to security groups only" when the [authoritative behavior](https://learn.microsoft.com/power-platform/admin/managed-environment-sharing-limits) excludes security-group sharing and limits sharing to individual users. Corrected the `bot-limitSharingMode` and `bot-maxLimitUserSharing` descriptions so violation/evidence output is accurate.
