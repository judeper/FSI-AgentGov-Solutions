#!/usr/bin/env python3
"""
AAM Deployment Orchestrator.

Creates all Dataverse components for Agent Access Governance Monitor:
- Shared option sets (zone, severity — reused from ACV if present)
- Tables (AccessBaseline, AccessValidationHistory, AccessViolation)
- Columns on each table
- Environment variables (operational parameters, alerting config)
...
def print_banner():
    ...
    print()
    print("=" * 70)
    print("  Agent Access Governance Monitor - Dataverse Deployment")
    print("=" * 70)
    print()
    print("  This script deploys AAM components to Dataverse:")
    print("    - Option sets (zone, severity — shared with ACV)")
    print("    - AccessBaseline table (org-owned, access snapshots)")
...
def deploy(
    ...
    """
    Deploy all AAM components to Dataverse.
...
def _print_post_deployment(is_dry_run: bool):
    ...
    print()
    print("\u2550" * 51)
    print("  Agent Access Governance Monitor \u2014 ", end="")
    ...
    if is_dry_run:
        ...
        print("     for all roles except System Administrator")
        print("  2. Connections: Bind connection references in Power Automate")
        print("     - Open make.powerapps.com \u2192 Solutions \u2192 Agent Access Monitor")
        print("     - Select each connection reference and click \"Set connection\"")
        print("  3. Verify: Run 'python deploy.py --dry-run' to confirm all components")
