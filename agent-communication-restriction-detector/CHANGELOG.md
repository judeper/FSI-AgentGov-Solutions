...
- **README technical re-verification (`Last Verified` 2026-05-25 → 2026-07-26).** Re-checked every Microsoft product claim in `README.md` against current Microsoft Learn documentation; no claim had drifted. Confirmed: Copilot Studio multi-agent orchestration still exposes child agents and connected agents; skills registration is still supported at 100 skills per agent; the published connector payload limit is still 5 MB; Microsoft Entra cross-tenant access settings still expose B2B collaboration and B2B direct connect inbound/outbound configuration; `Microsoft.PowerApps.Administration.PowerShell` still requires Windows PowerShell 5.x (incompatible with PowerShell 6.0+); and Power Platform still defines the Default, Sandbox, and Trial environment types used by the environment-filtering feature.
- **README solution-component tree corrected.** `scripts/Compare-CommRestrictionCompliance.ps1` ships with the solution but was missing from the component listing; added it.
