...
<#
.SYNOPSIS
    Verifies SHA-256 integrity of agent communication restriction evidence files.
...
.DESCRIPTION
    ...
    Evidence files produced by Export-CommViolationEvidence include SHA-256
...
.EXAMPLE
    .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath ".\evidence\acrd-evidence-All-20260210-143022.json"
...
.EXAMPLE
    Get-ChildItem .\evidence\acrd-evidence-*.json | ForEach-Object {
        .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath $_.FullName
    }

    Batch verification of all ACRD evidence files in a directory. Iterates through
...
.NOTES
    ...
    - acrd-evidence-{zone}-{timestamp}.json
    ...
    - Example: "abc123...  acrd-evidence-All-20260210-143022.json"
...
try {
    ...
    # Format: "{hash}  {filename}" (two spaces, standard checksum format)
    ...
    # Compute actual SHA-256 hash of evidence file
