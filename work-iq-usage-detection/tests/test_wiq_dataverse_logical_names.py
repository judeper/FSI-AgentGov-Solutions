"""Dataverse logical-name convention tests for work-iq-usage-detection.

Parses ``scripts/create_wiq_dataverse_schema.py`` to extract every Dataverse
...
argument and carrying the ``fsi_wiq_`` infix) and table names are excluded —
only column attribute names are checked.
...
SCHEMA_FILE = SOLUTION_ROOT / "scripts" / "create_wiq_dataverse_schema.py"
...
def _column_schema_names() -> list[str]:
    """Return the column SchemaNames declared in the WIQ schema generator."""
    text = _schema_text()
    if not text:
        return []
...
_NON_COLUMN_INFIX_RE = re.compile(r"fsi_(?:wiq|acv|cr)_")
