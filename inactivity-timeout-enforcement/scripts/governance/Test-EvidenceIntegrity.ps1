...
<#
.SYNOPSIS
    Verifies SHA-256 integrity of inactivity timeout compliance evidence files.
...
.DESCRIPTION
    ...
    Evidence files produced by Export-TimeoutComplianceEvidence include SHA-256
...
.EXAMPLE
    .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath ".\evidence\ite-evidence-All-20260209-143022.json"
...
.EXAMPLE
    Get-ChildItem .\evidence\ite-evidence-*.json | ForEach-Object {
        .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath $_.FullName
    }

    Batch verification of all ITE evidence files in a directory. Iterates through
    all matching JSON files and verifies each one against its companion hash file.

.EXAMPLE
    $isValid = .\Test-EvidenceIntegrity.ps1 `
        -EvidenceFilePath ".\evidence\ite-evidence-All-20260209-143022.json" `
        -Quiet
...
.NOTES
    ...
    - Example: "abc123...  ite-evidence-All-20260209-143022.json"
