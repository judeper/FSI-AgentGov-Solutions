...
- **Technical-accuracy review (Microsoft Learn):** Corrected the `docs/troubleshooting.md` "Authentication errors" resolution that told operators to grant the app registration the delegated `user_impersonation` Dataverse API permission. The integration authenticates as a confidential client using the client-credentials `<environment-url>/.default` scope; Dataverse access for managed identities and service principals is granted by adding the identity as an **Application User** with an assigned **security role** (a valid token alone returns `403`). `user_impersonation` is the public/interactive-client scope and does not apply. Now consistent with `PREREQUISITES.md`. Verified against [Use OAuth with Dataverse](https://learn.microsoft.com/en-us/power-apps/developer/data-platform/authenticate-oauth) and [Create an application user](https://learn.microsoft.com/en-us/power-platform/admin/manage-application-users#create-an-application-user).
- **Doc/script drift:** Updated the `docs/status-mapping.md` sample `fsi_notes` value to match the string actually written by `Sync-SolutionAssessments.ps1:378` (`"Automated: {SolutionName} {Version}. Run ID: {guid}. Status: {label}. Source: {entityset}, Timestamp: {iso}."`). The previous sample (`"Automated assessment via ACV v1.0.0…"`) drifted from the real prefix and version and would not match the `Automated:%` identification filter documented in `docs/score-calculator-update.md`.
