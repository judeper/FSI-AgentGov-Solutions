...
- Backported the helper dot-source parameter-preservation pattern to `Export-AuditValidationEvidence.ps1`, `Invoke-EnvironmentAuditValidation.ps1`, `Invoke-TenantAuditValidation.ps1`, `Start-EnvironmentValidationRunbook.ps1`, `Start-TenantValidationRunbook.ps1`, `Test-MailboxAudit.ps1`, `Test-PurviewRetention.ps1`, and `Test-UnifiedAuditLog.ps1`; additionally applied the same fix to `Invoke-EnvironmentDiscovery.ps1`. Added source-contract and behavioral coverage; live environment and tenant reruns succeeded.
- Fixed environment-validator dot-source loading for orchestrator execution: script-scope parameters in `Test-EnvironmentAudit.ps1` and `Test-EnvironmentRetention.ps1` are optional for loading, while function-scope mandatory contracts remain unchanged. Added behavioral coverage; the live environment path succeeded.
- Addressed the run-ID lifetime regression in environment orchestration and added tenant wrapper RunId output. Live environment and tenant results contained correlated RunIds and persisted history rows.
- Addressed baseline-status numeric normalization in `Compare-ValidationBaseline.ps1`; persisted regression and restoration checks mapped the Passed baseline correctly.
...
- Addressed app-only canary mailbox identity handling. Explicit shared-mailbox canary generation, attribute restoration, and later Unified Audit Log retrieval succeeded.
- Addressed the nested dot-source snapshot collision in tenant orchestration; live tenant output retained Zone3.
...
- Addressed the Power Apps compatibility-bound regression by pinning `Microsoft.PowerApps.Administration.PowerShell` to `2.0.180`; the final short-lived-secret fallback run succeeded and cleanup was verified.
- Completed private live validation: full ACV + ALCA schema deployment, certificate and short-lived-secret environment paths, certificate tenant path, persisted drift regression/restoration, retrieved Unified Audit Log canary, independently verified environment/tenant evidence hashes, separate Purview and Power Platform admin Playwright smokes, and exact-source Azure Automation PowerShell 7.4 jobs.
...
- Added `IsPrimaryName: true` to the primary-name `StringAttributeMetadata` in all ACM CreateEntity payloads (`AuditValidationHistory`, `EnvironmentRegistry`, `AuditEnvironmentCompliance`) after live Dataverse `0x80040203` failures. The full end-to-end schema deployment succeeded.
- Added explicit Dataverse metadata publication/readiness gates in ACV/ALCA schema deployment. The full canonical deployment rerun succeeded.
