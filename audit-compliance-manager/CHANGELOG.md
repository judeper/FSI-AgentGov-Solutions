...
- **Script auth:** Migrated `scripts/private/Connect-PowerPlatform.ps1` from the archived [`MSAL.PS`](https://github.com/AzureAD/MSAL.PS) module (last updated September 2023, flagged for migration in v1.0.5) to `Az.Accounts`, completing the migration tracked in v1.0.5. The three auth paths now use: (interactive) `Get-AzContext` + `Get-AzAccessToken -ResourceUrl`; (certificate) `Connect-AzAccount -ServicePrincipal -CertificateThumbprint` + `Get-AzAccessToken -ResourceUrl`; (secret, legacy dev-only) direct OAuth 2.0 client credentials grant via `Invoke-RestMethod`. All paths unwrap the SecureString token returned by Az.Accounts 5.x via Marshal BSTR. Removed the `# NOTE: MSAL.PS is archived` comment (migration is now done). Updated `#Requires` to add `Az.Accounts`. Matched the `Connect-EnvironmentDataverse.ps1` helper pattern from `action-confirmation-auditor`.
...
- **Invalid `AuditLogRecordType` value `PowerPlatformAdmin*` (tech-accuracy R3 · squad audit).** `Test-AuditLoggingCompliance.ps1` passed `-RecordType "PowerPlatformAdminActivity"` to `Search-UnifiedAuditLog`, and `Test-PurviewRetention.ps1` used the bare value `"PowerPlatformAdmin"` in its `$RequiredRecordTypes` coverage array, docstrings, and pass-reason output. Neither is a member of the `AuditLogRecordType` enum; the correct member is **`PowerPlatformAdministratorActivity`** (value 256). An invalid `-RecordType` combined with `-ErrorAction SilentlyContinue` makes `Search-UnifiedAuditLog` return zero rows silently, so the check reported Power Platform administrator activity as covered/absent without ever querying it; in `Test-PurviewRetention.ps1` the bad value also flowed into the remediation-command generator, which would have emitted an invalid `New-UnifiedAuditLogRetentionPolicy -RecordTypes PowerPlatformAdmin`. Corrected every occurrence to `PowerPlatformAdministratorActivity`; this supersedes the earlier change that introduced `PowerPlatformAdminActivity`. Source: Microsoft Learn [Office 365 Management Activity API schema](https://learn.microsoft.com/en-us/office/office-365-management-api/office-365-management-activity-api-schema#auditlogrecordtype) (AuditLogRecordType 256 = `PowerPlatformAdministratorActivity`).
- **Stale `UnifiedAuditLogRetentionDuration` enum names in `Test-PurviewRetention.ps1` docstring (tech-accuracy R3 · squad audit).** The `.DESCRIPTION` retention-duration mapping still listed non-existent members (`OneYear`, `TwoYears`, `ThreeYears`, `FiveYears`, `SevenYears`). The executable map and `Get-RequiredRetentionDuration` were corrected in the `RetentionDuration` entry below; this completes the same correction in documentation. Replaced with the valid members (`ThreeMonths`, `SixMonths`, `NineMonths`, `TwelveMonths`, `TenYears`) and noted the enum has no member between `TwelveMonths` (365 days) and `TenYears` (3650 days). Source: Microsoft Learn [`New-UnifiedAuditLogRetentionPolicy`](https://learn.microsoft.com/en-us/powershell/module/exchange/new-unifiedauditlogretentionpolicy) (`-RetentionDuration`).
- **Inaccurate per-mailbox audit "override" detection (`Test-MailboxAudit.ps1`):** The Step 3 check sampled mailboxes and flagged `AuditEnabled=$false` as a per-mailbox override. Per Microsoft Learn ([Manage mailbox auditing](https://learn.microsoft.com/en-us/purview/audit-mailboxes)), when *mailbox auditing on by default* is turned on it overrides the per-mailbox `AuditEnabled` property — `Get-Mailbox` always returns `AuditEnabled=$true` and setting it to `$false` is ignored — so the old check could never detect a real exclusion. Replaced it with a `Get-MailboxAuditBypassAssociation` check (the documented exclusion mechanism), flagging accounts where `AuditBypassEnabled=$true`. Result check renamed `SampleMailboxOverrides` → `MailboxAuditBypassAssociations`; Warning remediation now points to `Set-MailboxAuditBypassAssociation -AuditBypassEnabled $false`. Docstring corrected accordingly.
