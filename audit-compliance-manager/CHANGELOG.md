...
- **Invalid `RecordType` strings for Power Platform admin events (third-pass tech-accuracy audit):** `Test-PurviewRetention.ps1` (lines 20, 30, 181, 486) declared `PowerPlatformAdmin` as a "critical record type for AI agent governance" and `Test-AuditLoggingCompliance.ps1:232` passed `PowerPlatformAdminActivity` to `Search-UnifiedAuditLog -RecordType`. Per Microsoft Learn ([Office 365 Management Activity API schema — AuditLogRecordType](https://learn.microsoft.com/en-us/office/office-365-management-api/office-365-management-activity-api-schema)), neither name exists in the enum. The valid values are `PowerPlatformAdministratorActivity` (value 256, "Power Platform Administrator activity events") and `PowerPlatformAdminDlp` (value 187, "Microsoft Power Platform DLP (Preview)"). Both unrecognised names cause `Search-UnifiedAuditLog` to silently return zero results, so the coverage-gap check could pass with a false "covered" status while real Power Platform admin events went undetected. Replaced both occurrences with `PowerPlatformAdministratorActivity`. The earlier fix in CHANGELOG entry #11 ("`PowerAppsApp` → `PowerPlatformAdminActivity`") corrected the operator name but landed on a still-invalid enum value.
- **Stale enum-list in `Test-PurviewRetention.ps1` `.NOTES` block (lines 89-98):** The block listed `OneYear / TwoYears / ThreeYears / FiveYears / SevenYears` as valid `UnifiedAuditLogRetentionDuration` values, contradicting the already-correct `$RetentionDurationMap` and `Get-RequiredRetentionDuration` implementation. Microsoft Learn ([`New-UnifiedAuditLogRetentionPolicy`](https://learn.microsoft.com/en-us/powershell/module/exchangepowershell/new-unifiedauditlogretentionpolicy)) confirms the enum only accepts `ThreeMonths`, `SixMonths`, `NineMonths`, `TwelveMonths`, `TenYears`. The block now mirrors the in-code comment and explicitly notes no enum value exists between `TwelveMonths` (365 days) and `TenYears` (3650 days). This was a downstream-propagation gap left behind by the prior `[Unreleased]` entry that fixed the runtime enum.
