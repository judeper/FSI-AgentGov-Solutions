...
- Fixed all `BooleanAttributeMetadata` create-attribute payloads in `create_dataverse_schema.py` (column `fsi_OverrideInclude`) and `create_audit_compliance_schema.py` (columns `fsi_AuditEnabled`, `fsi_DataverseAuditEnabled`) to include the full Dataverse Web API Boolean-column create contract: `"AttributeType": "Boolean"`, `"AttributeTypeName": {"Value": "BooleanType"}`, and an `"OptionSet"` containing `BooleanOptionSetMetadata` with `OptionSetType=Boolean`, `TrueOption Value=1`, and `FalseOption Value=0` with labels. Live evidence (2026-07-14, commit 40f92ce): ACV had successfully created all AuditValidationHistory custom columns and EnvironmentRegistry columns through `fsi_lastvalidated`; the POST for the next column, `fsi_OverrideInclude`, returned HTTP 400 with body `{"error":{"code":"0x80048403","message":"The option set for a Boolean attribute must have two options for true and false values."}}`. Authoritative source: Microsoft Learn "Create a Boolean column using the Web API." Added a private `_boolean_optionset()` helper in each schema script that returns a fresh `BooleanOptionSetMetadata` dict per call so no two columns share a mutable object. Existing `DefaultValue` values (`False` for all three columns) are preserved. Added pytest regression coverage in `tests/test_boolean_attribute_contract.py` that enumerates all three current Boolean definitions across both schema scripts and asserts the full create contract, distinct `OptionSet` identity, and `DefaultValue` preservation.
- Fixed `ACVClient.create_attribute` and `ALCAClient.create_attribute` handling of `GlobalOptionSet@odata.bind` Name alternate-key values (`/GlobalOptionSetDefinitions(Name='...')`) at the Dataverse create-attribute endpoint. Live evidence (2026-07-14, commit 363ad1d): one-shot service-principal POST with a corrected `PicklistAttributeMetadata` payload returned HTTP 500 with response body `{"error":{"code":"0x0","message":"Guid should contain 32 digits with 4 dashes (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)."}}`. Microsoft Learn documents the Name form but uses the MetadataId GUID form in its primary create-column example. Both clients now resolve the option set by Name, require a valid UUID `MetadataId`, deep-copy the payload, and POST `/GlobalOptionSetDefinitions(<normalized-uuid>)`; malformed bindings fail before POST. Non-choice attributes, valid MetadataId bindings, and dry-run behavior remain unchanged. Added ACV/ALCA regression coverage in `tests/test_global_optionset_bind_resolution.py`.
...
- Fixed drift-baseline sequencing in ACM runbook wrappers: `Compare-ValidationBaseline.ps1` now accepts optional `CurrentRunId` and excludes `fsi_runid` from baseline selection when provided, with OData single-quote escaping. `Start-EnvironmentValidationRunbook.ps1` and `Start-TenantValidationRunbook.ps1` now pass the current run ID to baseline comparisons, and `Invoke-TenantAuditValidation.ps1` now exposes `RunId` in orchestrator output for tenant drift exclusion wiring. Added targeted Pester regression coverage in `scripts/Validators.Tests.ps1` for URI contract, first-run behavior, regression/restoration behavior, and wrapper wiring contracts.
- Backported the helper dot-source parameter-preservation pattern to `Export-AuditValidationEvidence.ps1`, `Invoke-EnvironmentAuditValidation.ps1`, `Invoke-TenantAuditValidation.ps1`, `Start-EnvironmentValidationRunbook.ps1`, `Start-TenantValidationRunbook.ps1`, `Test-MailboxAudit.ps1`, `Test-PurviewRetention.ps1`, and `Test-UnifiedAuditLog.ps1`; additionally applied the same script-scope snapshot/restore fix to `Invoke-EnvironmentDiscovery.ps1` to prevent helper param-block clobbering of non-empty caller values (including Dataverse URL and runbook auth inputs). Added source-contract and behavioral probe coverage in `scripts/Validators.Tests.ps1`. Live tenant rerun remains pending.
- Fixed environment-validator dot-source loading for orchestrator execution: script-scope parameters in `Test-EnvironmentAudit.ps1` (`EnvironmentUrl`, `AccessToken`) and `Test-EnvironmentRetention.ps1` (`EnvironmentUrl`, `AccessToken`, `DataverseUrl`, `CentralAccessToken`, `Zone`) are now optional at script scope to allow argument-free dot-sourcing, while function-scope mandatory contracts remain unchanged for runtime invocation. `scripts/Validators.Tests.ps1` now covers both validators' direct-invocation blocks and adds a behavioral dot-source test that loads sanitized copies with no arguments, verifies function availability, and asserts mandatory function parameters are preserved. Live rerun evidence remains pending.
- Addressed finding 21 run-ID lifetime regression in environment orchestration: `Invoke-EnvironmentAuditValidation.ps1` now generates and prints `$runId`/`$timestamp` after helper and validator dot-sourcing restores script-scope variables, so the dot-sourced `Write-ValidationResult.ps1` script param can no longer clobber the local run ID before write calls. The same single RunId is retained in results output and all three environment `Write-ValidationResult` parameter sets. `Start-TenantValidationRunbook.ps1` now includes `RunId = $validationResults.RunId` in its final output object for parity with environment-wrapper evidence correlation. Added targeted source-contract coverage in `scripts/Validators.Tests.ps1`. Live rerun evidence remains pending.
- Addressed finding 22 baseline-status mapping defect in `Compare-ValidationBaseline.ps1`: Dataverse `fsi_severity` baseline values now normalize to `[int]` before reverse-map lookup and severity comparison so persisted `[long]100000000` and `'100000000'` values map to `Passed` correctly. Unknown or non-numeric baseline severities now throw precise normalization errors that flow through the existing fail-open path (`DriftDetected = $true`, `Error` populated) with no silent fallback status. Added targeted Pester coverage in `scripts/Validators.Tests.ps1` for Int64, numeric-string, and invalid-severity behaviors. Live rerun evidence remains pending.
- Addressed finding 23 validator-load and app-only parameter-set defects: `Test-PurviewRetention.ps1` now keeps script-scope `Zone` optional for argument-free dot-sourcing while preserving mandatory function-scope `Zone`; `Test-UnifiedAuditLog.ps1` now adds `Interactive` to `Connect-AuditServices` only when true so service-principal certificate parameters no longer conflict with an explicit `Interactive=$false` binding. Added regression coverage in `scripts/Validators.Tests.ps1` for Purview no-arg dot-source + mandatory function contract and Unified Audit conditional Interactive splatting.
- Addressed finding 24 app-only canary mailbox identity handling: `Test-UnifiedAuditLog.ps1` now supports optional `CanaryMailboxIdentity` (script + function scope), forwards it to `New-CanaryEvent` when provided, and returns a clear Warning (without calling `New-CanaryEvent`) when service-principal validation runs without an explicit mailbox identity. Added `CanaryMailboxIdentity` plumbing through `Invoke-TenantAuditValidation.ps1` and `Start-TenantValidationRunbook.ps1`, plus wiring/behavior regression coverage in `scripts/Validators.Tests.ps1`. Live evidence captured explicit canary generate+restore success with a provided shared mailbox, while canary retrieval remained pending after 12 polls (not yet a pass state).
- Addressed finding 25 nested dot-source snapshot collision in tenant orchestration: `Invoke-TenantAuditValidation.ps1` now stores caller script-scope values in a caller-owned snapshot variable (`$tenantOrchestratorSafeVars`) and restores from that variable after validator dot-sourcing, preventing nested validator reuse of `$dotSourceSafeVars` from clearing orchestrator `Zone`. `scripts/Validators.Tests.ps1` now supports per-row snapshot variable names in the dot-source source-contract matrix, adds a behavioral nested-collision probe that reproduces shared-name clobbering and verifies unique-snapshot survival for Zone3, and adds a source contract confirming tenant results/zone mapping consume the restored `Zone`. Live rerun evidence remains pending.
- Fixed explicit zero-second canary smoke configuration in `Start-TenantValidationRunbook.ps1`: `CanaryWaitSeconds` is now forwarded based on `$PSBoundParameters.ContainsKey(...)` rather than truthiness, so `-CanaryWaitSeconds 0` reaches `Test-UnifiedAuditLog` instead of silently reverting to the 300-second default.
- Fixed tenant validation history persistence by forwarding `DataverseUrl` from `Start-TenantValidationRunbook.ps1` into `Invoke-TenantAuditValidation.ps1`; prior certificate runs completed validator and drift output but produced a hash-valid tenant evidence export with zero records because the orchestrator was not given the central Dataverse target.
- Addressed finding 28 Power Apps compatibility-bound regression: the six ACM scripts that require `Microsoft.PowerApps.Administration.PowerShell` now pin both `ModuleVersion` and `MaximumVersion` to `2.0.180` so minimum-only `#Requires` does not admit `2.0.217` for ACM's validated app-secret fallback path. ACM evidence showed `Add-PowerAppsAccount` app-secret auth failed on `2.0.217` with `AADSTS7000215`, while the same short-lived secret succeeded on `2.0.180`; certificate-based validation on `2.0.217` succeeded. Live fallback rerun remains pending until after commit.
