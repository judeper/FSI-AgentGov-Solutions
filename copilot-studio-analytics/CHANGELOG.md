...
- **Stale Copilot Studio Application Insights navigation path in `prerequisites.md` (Per-Agent Setup, step 2).** The instructions said to navigate to *Settings → Diagnostics* and then enable Application Insights as a separate toggle. Per Microsoft Learn ([Capture telemetry with Application Insights](https://learn.microsoft.com/en-us/microsoft-copilot-studio/advanced-bot-framework-composer-capture-telemetry)), the current step is *Settings → **Advanced*** and the connection string is entered directly within the **Application Insights** section (no discrete enable toggle). The previous wording broke onboarding because no "Diagnostics" tab exists, blocking the telemetry pipeline this entire analytics solution depends on. Aligned with `agent-observability-foundation/prerequisites.md`, which already used the correct path. (technical accuracy review vs Microsoft Learn) `docs/agent-assisted-hours-methodology.md` ("Maker-Led Inputs (Viva-Only Limitation)") and `docs/viva-insights-parity-matrix.md` described the per-topic/per-action maker time-savings inputs as a Viva Insights-exclusive feature "stored in Viva Insights backend services" with "no public API." Per Microsoft Learn, this is the native Copilot Studio **Savings calculator** on each agent's **Analytics** page — makers enter savings *per run* or *per tool*, it is gated only by the environment-level **Disable money saving rules** admin setting (Power Platform admin center), and it does **not** require a Viva Insights license (https://learn.microsoft.com/microsoft-copilot-studio/analytics-cost-savings). Reframed both docs: the feature is Copilot Studio-native, CSA's actual limitation is that its Tier 1 sync does not ingest those estimates (they are not part of the `msdyn_botsession` records CSA reads), and CSA uses config.yml defaults as the alternative. Also corrected the parity-matrix row that labeled "Copilot Studio embedded analytics" as Viva-only and added a "Copilot Studio native" parity status. (technical accuracy review vs Microsoft Learn)
- **Critical (second-pass) — autonomous-agent classification queried a non-existent `botcomponent` column.** `get_agent_classifications()` selected and read `_botid_value` on the `botcomponents` entity set, and `docs/dataverse-data-sources.md` documented the lookup as `botid` / filtered on `_botid_value`. The `botcomponent` table has no `botid` column; its parent-Copilot lookup is `parentbotid` (Targets: `bot`), exposed in the Web API as `_parentbotid_value` (https://learn.microsoft.com/power-apps/developer/data-platform/reference/entities/botcomponent). Selecting/filtering `_botid_value` returns HTTP 400, so every agent fell back to "Conversational" and autonomous metrics read zero. Corrected the `$select`/read in `sync_dataverse_sessions.py` and the lookup name + example queries in `docs/dataverse-data-sources.md`.
