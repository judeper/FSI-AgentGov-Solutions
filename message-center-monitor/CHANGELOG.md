...
### Fixed
- **Technical accuracy re-verification 2026-07-26 (vs Microsoft Learn):** Re-checked every Microsoft product claim in `README.md` against current official documentation and corrected five that had drifted. (1) Entra portal path is now **Entra ID > App registrations > New registration** (was "Applications > App registrations"). (2) Power Platform admin center application-user path now includes the **Manage** navigation step. (3) DLP path is now **Security > Data and privacy > Data policy** (was "Data policies"). (4) Removed the reference to a Power Automate `stripHtml()` expression — no such function exists in the Logic Apps / Power Automate workflow definition language; replaced with the Content Conversion connector's **Html to text** action, workflow string functions, or PowerShell-side conversion. (5) Connector display name is now **HTTP with Microsoft Entra ID (preauthorized)**, matching `docs/flow-configuration.md`. The same PPAC navigation corrections (2) and (3) were applied to `docs/setup-checklist.md`, which defers to `README.md` for these details. Also noted that Graph `category` and `severity` are evolvable enums carrying an `unknownFutureValue` sentinel. Verified-accurate and unchanged: Graph v1.0 `GET /admin/serviceAnnouncement/messages`; `ServiceMessage.Read.All` as the sole (least-privileged) permission; `ServiceHealth.Read.All` scoping for `healthOverviews`/`issues`; all `serviceUpdateMessage` property and enum names; the Office 365 connectors retirement rollout window (2026-05-18 to 2026-05-22); the absence of Message Center push/webhook support in Graph change notifications; the federated-credential and certificate-upload portal paths; the `Update-MgApplication -ApplicationId <object-id> -KeyCredentials` pattern; and Power Automate Premium licensing for the Microsoft Dataverse and HTTP with Microsoft Entra ID connectors.
- **Technical accuracy review (vs Microsoft Learn):** Corrected the Office 365 connectors retirement date in `README.md`, `docs/teams-integration.md`, and `.ralph-config.json`. The previously stated `2026-03-31` (and `2026-Q1`) was a superseded deadline; per Microsoft's [official announcement](https://devblogs.microsoft.com/microsoft365dev/retirement-of-office-365-connectors-within-microsoft-teams/), the final deprecation rollout began **2026-05-18** and completed **2026-05-22** (after extensions through 2026-03-31 then 2026-04-30). Clarified that Phase 1 posts to a Teams Workflows incoming webhook and Phase 3 uses the native "Post card in a chat or channel" connector — neither uses retired Office 365 connectors. Verified-accurate and unchanged: Graph v1.0 endpoint `GET /admin/serviceAnnouncement/messages`, application permission `ServiceMessage.Read.All`, and all `serviceUpdateMessage` property/enum names (`category`, `severity`, `services`, `tags`, `isMajorChange`, `actionRequiredByDateTime`, `hasAttachments`, etc.).
