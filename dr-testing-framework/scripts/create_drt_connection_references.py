#!/usr/bin/env python3
"""
Create connection references for DR Testing Framework.

Connection references enable Power Automate flows to access Dataverse
and Teams for DR test scheduling, execution, and notification.
"""
...
CONNECTION_REFS = [
    {
        "logical_name": "fsi_cr_dataverse_drtesting",
        "display_name": "Dataverse - DR Testing",
        "connector": "shared_commondataserviceforapps",
        "description": (
            "Dataverse connector for DR test result storage, "
            "evidence queries, and schema operations"
        ),
    },
    {
        "logical_name": "fsi_cr_teams_drtesting",
        "display_name": "Teams - DR Testing",
        "connector": "shared_teams",
        "description": (
            "Teams connector for DR test completion notifications "
            "and failure alert cards"
        ),
    },
]
...
def create_connection_references(
    ...
    """
    Create connection references for DR Testing Power Automate flows.
...
Connection references created:
  - fsi_cr_dataverse_drtesting (Dataverse connector)
  - fsi_cr_teams_drtesting (Teams connector)
...
Examples:
  ...
  python create_drt_connection_references.py \\
      --tenant-id <tenant-id> \\
