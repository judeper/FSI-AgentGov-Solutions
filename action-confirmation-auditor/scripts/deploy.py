#!/usr/bin/env python3
"""Deploy Action Confirmation Auditor infrastructure to Dataverse.

Orchestrates the full deployment pipeline:
  1. Test Connection -- validate Dataverse API access
  2. Dataverse Schema -- tables, columns, shared and ACA option sets
...
1. SECURITY -- Configure fsi_ActionScanRun security role:
   - Navigate to Settings > Security > Security Roles
   - Remove Write and Delete privileges for fsi_ActionScanRun
   - This supports immutable audit records (FINRA 4511, SEC 17a-3)

2. CONNECTIONS -- Bind connection references in Power Automate:
   - fsi_cr_dataverse_actionconfirmationauditor       -> Select Dataverse connection
   - fsi_cr_office365_actionconfirmationauditor       -> Select Office 365 connection
   - fsi_cr_teams_actionconfirmationauditor           -> Select Teams connection
...
def run_deployment(
    ...
    print()
    print("=" * 60)
    print("Action Confirmation Auditor -- Dataverse Deployment")
    print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  ACA_TENANT_ID        Azure AD tenant ID\n"
