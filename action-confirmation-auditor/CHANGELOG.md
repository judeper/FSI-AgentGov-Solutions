...
- **`README.md` technical-accuracy re-verification (2026-07-26)**, drift issue #334. Re-checked every Microsoft product claim against current Microsoft Learn and corrected four that had drifted:
  - **Copilot Studio "actions" → "tools".** The capabilities an agent invokes are now **tools**, added from the agent's **Tools** page as a connector (prebuilt/custom), agent flow, prompt, REST API, Model Context Protocol server, or computer use. Confirmation is a per-tool **Ask the end user before running** setting under the tool's **Details** section, defaulting to **No**. The topic-level **Action** node name is unchanged. The old intro listed "connector calls, cloud flows, plugins, HTTP requests"; *plugins* is not part of the current Copilot Studio tool taxonomy. Added a "Copilot Studio terminology" subsection mapping ACA's internal "action" vocabulary (Dataverse columns, severity classes) onto the current product names. (verified against `https://learn.microsoft.com/microsoft-copilot-studio/add-tools-custom-agent`)
  - **"User-Defined Action Messages" is an ACA term, not a Microsoft setting.** The feature bullet implied a Copilot Studio setting of that name. Microsoft documents no such setting; the equivalent controls are the per-tool **Ask the end user before running** toggle and the tool **Description** shown at the confirmation prompt. Bullet reworded to label UDAM as ACA-internal and name the real settings. No behavioral change to `governance/Test-UserDefinedActionMessages.ps1`. (same source)
  - **Azure Automation Run As accounts: "deprecated" → retired.** Run As accounts were **retired on 30 September 2023** and are replaced by managed identities; "deprecated" understated the status. (verified against `https://learn.microsoft.com/azure/automation/migrate-run-as-accounts-managed-identity`)
  - **"Purview AI Hub" is obsolete branding.** The current product is Microsoft Purview **Data Security Posture Management (DSPM)**, with AI activity surfaced via AI observability and Activity Explorer; the prior releases now appear as *DSPM for AI (classic)* and *Data Security Posture Management (classic)*. Feature bullet renamed to **Microsoft Purview DSPM Integration** and now states that `Get-PurviewAIHubEvidence.ps1` keeps the historical file name. (verified against `https://learn.microsoft.com/purview/data-security-posture-management-learn-about` and `https://learn.microsoft.com/purview/dspm-for-ai`)
  - **Prerequisites completeness.** "Power Platform Admin or Entra Global Admin permissions" is no longer sufficient on its own for Dataverse-scoped scanning: Microsoft no longer automatically assigns the Dataverse **System Administrator** role to Power Platform Administrator or Dynamics 365 Administrator accounts. Added that role as an explicit per-environment prerequisite. (verified against `https://learn.microsoft.com/power-platform/admin/manage-high-privileged-admin-roles`)
  - Re-checked and confirmed **still accurate**: *Microsoft 365 E5 Compliance* remains a current product name; *Power Automate Premium* remains the current user license name; *Power Platform Administrator* and *Global Administrator* remain current Entra role names; the Control 1.23 note's product mechanisms (Entra Authentication Contexts, Conditional Access, phishing-resistant MFA) are unchanged.
  - No regulatory or compliance mapping was added, removed, or reinterpreted. `**Last Verified:**` advanced from 2026-05-25 to 2026-07-26.
