...
- **Dataverse-token acquisition examples corrected (tech-accuracy R3 · squad audit)** (`scripts/private/Get-SSCValidationResults.ps1`, `scripts/private/Get-DataverseThreshold.ps1`): Both `.EXAMPLE` blocks showed `Connect-MgGraph -Scopes "https://contoso.crm.dynamics.com/.default"` to obtain a token for a subsequent Dataverse Web API call. `Connect-MgGraph` only issues Microsoft Graph-audience tokens, which the Dataverse Web API rejects (HTTP 401), so the examples could not work as written. Replaced with `Get-AzAccessToken -ResourceUrl "https://contoso.crm.dynamics.com"` — the method Microsoft documents for acquiring a Dataverse token — piped through `ConvertFrom-SecureString -AsPlainText` (the `Token` property is a `SecureString` by default in Az.Accounts 5.x) and passed via `-AccessToken`. Consistent with the "Get-AzAccessToken SecureString caveat" note above. Sources: Microsoft Learn [Authenticate to Dataverse with the Web API](https://learn.microsoft.com/power-apps/developer/data-platform/webapi/authenticate-web-api) ("use the Get-AzAccessToken command to request an access token for your Dataverse organization URI resource"), [Get-AzAccessToken](https://learn.microsoft.com/en-us/powershell/module/az.accounts/get-azaccesstoken).
