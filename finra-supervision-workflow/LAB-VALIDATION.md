...
| `Get-SupervisoryReviewPolicyV2` available via `Connect-IPPSSession` (README & flow-configuration.md API fallback note; communication-compliance-setup.md Steps 5-6) | **Confirmed**: functional in Security & Compliance PowerShell. The legacy non-V2 `Get-SupervisoryReviewPolicy` page now returns 404 on Microsoft Learn and is superseded by V2; second-pass fix §4 replaces the remaining non-V2 references in communication-compliance-setup.md | [Get-SupervisoryReviewPolicyV2](https://learn.microsoft.com/powershell/module/exchangepowershell/get-supervisoryreviewpolicyv2) |
...
| 2 | `docs/prerequisites.md` `az keyvault create` example used `--enable-soft-delete true`. Soft delete is always enabled and cannot be disabled, so this flag was removed from `az keyvault create`; passing it now fails with "unrecognized arguments". | **Command-existence (would fail at runtime)** | Removed `--enable-soft-delete true`; added `--retention-days 90` for the soft-delete retention window. Verified against [az keyvault create reference](https://learn.microsoft.com/cli/azure/keyvault#az-keyvault-create) (flag absent from parameter list). |
| 3 | `docs/communication-compliance-setup.md` Steps 5-6 used the deprecated `Get-SupervisoryReviewPolicy` cmdlet, whose Microsoft Learn page now returns 404. README and flow-configuration.md already used the V2 cmdlet. | **Command-existence (deprecated/undocumented cmdlet)** | Replaced both occurrences with `Get-SupervisoryReviewPolicyV2`. Verified against [Get-SupervisoryReviewPolicyV2 reference](https://learn.microsoft.com/powershell/module/exchangepowershell/get-supervisoryreviewpolicyv2). |

A second-pass command-existence audit (2026-06-04) added gaps #2 and #3 above — two CLI/cmdlet references that would fail at runtime against current Microsoft tooling. The Python scripts (`auth.py`, `deploy.py`, `export_supervision_evidence.py`), Dataverse Web API routes/columns, schema docs, security-role matrix, flow specifications, and DAX measures were re-derived independently and remain accurate and internally consistent.
