...
* `solutions.json` (committed at repo root) — mirrors the framework
  ...
  inventory counts (`counts.total`, `counts.live`, `counts.preview`) so
...
statistics and the top-level `solutions` object for per-solution metadata.
Generated Markdown summaries (`README.md`, `site-docs/solutions/index.md`,
etc.) are human-readable projections only and MUST NOT be parsed for counts.
...
def emit_readme_table(manifests: dict[str, dict]) -> str:
    ...
    rows.append("")
    rows.append(
        "Downstream consumers should treat `solutions.json` (`counts` + `solutions`) "
        "as the authoritative machine-readable inventory export; this README block "
        "is a generated summary only."
    )
    rows.append("")
