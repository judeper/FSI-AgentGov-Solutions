#!/usr/bin/env python3
"""
Create connection references for Agent Sharing Access Restriction Detector.

Connection references enable Power Automate flows to access Dataverse, Teams,
Approvals, and HTTP Premium for sharing restriction detection and remediation.
"""
...
CONNECTION_REFS = [
    {
        "logical_name": "fsi_cr_dataverse_sharingrestriction",
        "display_name": "Dataverse - ASARD",
        "connector": "shared_commondataserviceforapps",
        "description": "Dataverse connector for ASARD compliance persistence",
    },
    {
        "logical_name": "fsi_cr_teams_sharingrestriction",
        "display_name": "Teams - ASARD",
        "connector": "shared_teams",
        "description": "Teams connector for sharing restriction alert notifications",
    },
    {
        "logical_name": "fsi_cr_approvals_sharingrestriction",
        "display_name": "Approvals - ASARD",
        "connector": "shared_approvals",
        "description": "Approvals connector for ASARD remediation workflows",
    },
    {
...
def create_connection_references(client: DataverseClient, dry_run: bool = False) -> dict:
    """
    Create connection references for ASARD Power Automate flows.
...
def main() -> None:
    """CLI entry point for ASARD connection reference creation."""
    parser = argparse.ArgumentParser(
        description="Create connection references for Agent Sharing Access Restriction Detector",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Connection references created:
  - fsi_cr_dataverse_sharingrestriction (Dataverse connector)
  - fsi_cr_teams_sharingrestriction (Teams connector)
  - fsi_cr_approvals_sharingrestriction (Approvals connector)
  - fsi_cr_http_sharingrestriction (HTTP Premium connector)
...
Examples:
  ...
  python create_asard_connection_references.py \\
      --tenant-id <tenant-id> \\
  ...
  python create_asard_connection_references.py \\
    ...
    parser.add_argument(
        ...
        help="Microsoft Entra tenant ID (or ASARD_TENANT_ID env var)"
    )
    parser.add_argument(
        "--client-id",
        default=os.environ.get("ASARD_CLIENT_ID"),
        help="Service Principal application ID (or ASARD_CLIENT_ID env var)"
    )
    parser.add_argument(
        "--environment-url",
        default=os.environ.get("ASARD_ENVIRONMENT_URL"),
        help="Dataverse environment URL (or ASARD_ENVIRONMENT_URL env var)"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
