...
def main() -> None:
    ...
    if not args.tenant_id and not args.interactive:
        parser.error(
            "tenant-id required (set FUS_TENANT_ID env var, use --tenant-id, "
            "or use --interactive)"
        )
    if not args.url:
        parser.error(
            "url required (set FUS_DATAVERSE_URL env var or use --url)"
        )
