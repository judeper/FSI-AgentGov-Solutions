...
<#
.SYNOPSIS
    Verifies SHA-256 integrity of RAG source validation evidence files.
...
.DESCRIPTION
    ...
    Evidence files produced by Export-ValidationEvidence include SHA-256
...
.EXAMPLE
    .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath ".\evidence\rsv-evidence-20260315-143022.json"
...
.EXAMPLE
    Get-ChildItem .\evidence\rsv-evidence-*.json | ForEach-Object {
        .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath $_.FullName
    }

    Batch verification of all RSV evidence files in a directory. Iterates through
    all matching JSON files and verifies each one against its companion hash file.

.EXAMPLE
    $isValid = .\Test-EvidenceIntegrity.ps1 `
        -EvidenceFilePath ".\evidence\rsv-evidence-20260315-143022.json" `
        -Quiet
...
.NOTES
    ...
    - Example: "abc123...  rsv-evidence-20260315-143022.json"
