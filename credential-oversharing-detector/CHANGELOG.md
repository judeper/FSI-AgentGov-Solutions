...
> **Lab-validation findings (FSI-AgentGov-Lab live deploy on autojude, 2026-06-15):** the five live-deploy bug-class fixes below were surfaced when the v2.1.1 + `[Unreleased]` schema additions were deployed end-to-end to a real Dataverse tenant. All five were **masked by `--dry-run`, `--output-docs`, and v2.1.1 static validation** (they only surface on a real POST with the Dataverse Web API deserializer in play). Fixes mirror proven patterns from the ASARD / AAM solutions where applicable. Detailed evidence in `LAB-VALIDATION.md` §7 (lab-side) and the [FSI-AgentGov-Lab PR #17](https://github.com/judep_microsoft/FSI-AgentGov-Lab/pull/17) merge.

- **Live deploy bug — option-set `@odata.type` missing:** All six global option-set definitions in `create_cod_dataverse_schema.py` (`fsi_acv_zone`, `fsi_COD_violationtype`, `fsi_COD_violationstatus`, `fsi_COD_severity`, `fsi_COD_scanstatus`, `fsi_COD_exceptionstatus`) were rejected at live deploy with `0x80048d19 "Invalid property 'Options' was found in entity 'Microsoft.Dynamics.CRM.OptionSetMetadataBase'"` because the derived `Microsoft.Dynamics.CRM.OptionSetMetadata` type discriminator was missing from the POST body. Fix mirrors the proven ASARD/AAM pattern: added `"@odata.type": "Microsoft.Dynamics.CRM.OptionSetMetadata"` to every option-set definition.
- **Live deploy bug — `CreateEntity` missing `PrimaryAttribute` + `CreateAttribute` missing `AttributeType`:** `create_tables()` POSTed entity metadata without an `EntityMetadata` `@odata.type` discriminator AND without marking the primary-name attribute on the `Attributes` list. Dataverse returned `HTTP 400 "Required field 'PrimaryAttribute' is missing for RequestName='CreateEntity'"` on the first table create. Likewise `create_columns()` did not inject `AttributeType`/`AttributeTypeName` from the column-level `@odata.type` discriminator (which `CreateAttribute` requires explicitly). Ported the ASARD `_ATTRIBUTE_TYPE_NAMES` dict + `_normalize_attribute` helper, then patched `create_tables()` to `setdefault("@odata.type", "Microsoft.Dynamics.CRM.EntityMetadata")`, walk the `Attributes` list to mark `IsPrimaryName=True` on the primary-name attribute, and run `_normalize_attribute` per attribute; and patched `create_columns()` to run `_normalize_attribute` on each column before POST.
- **Live deploy bug — `environmentvariablevalues` PascalCase nav-prop + missing `schemaname`:** `create_cod_environment_variables.py` POSTed env-var values with `environmentvariabledefinitionid@odata.bind` (lowercase) and no `schemaname` field, both of which return HTTP 400 from the `environmentvariablevalues` entity set. All 11 env-var value creates failed and rolled back their parent definitions. Fix mirrors the proven AAM/ASARD pattern (2026-06-09): use PascalCase `EnvironmentVariableDefinitionId@odata.bind` AND add `"schemaname": schemaname,` to the value payload.
- **Live deploy bug — `Compare-OAuthScopeBaseline.ps1` missing `Content-Type` header on POST:** All POSTs to `fsi_credentialviolations` returned `0x80060888 "POST request requires Content-Type application/json"` because the headers hashtable was missing the `Content-Type` key (only set `Authorization`, `Accept`, `OData-Version`). The read (GET) path worked, which masked the bug from any read-only / dry-run probe. Added `'Content-Type' = 'application/json'` to the headers hashtable.
- **Live deploy bug — `Compare-OAuthScopeBaseline.ps1` not populating `fsi_detectedat` + scope columns on the violation it writes:** Violation rows were created with `fsi_detectedat = null`, so `Export-CredentialEvidence.ps1` — which filters violations by `fsi_detectedat ge X and le Y` date range — silently excluded every Compare-written violation from auditor evidence. Additionally the row lacked `fsi_approvedscopes` and `fsi_actualscopes`, so the JSON evidence carried only the recommendation text, not the actual scope diff. Added `fsi_detectedat = (Get-Date).ToUniversalTime().ToString('o')` to the value payload. Added `Approved` and `Actual` arrays to the result PSCustomObject in the comparison loop, then `fsi_approvedscopes = ($result.Approved -join ' ')` + `fsi_actualscopes = ($result.Actual -join ' ')` to the persist payload. **Companion fix:** the same `fsi_detectedat`-missing bug existed in `Test-AgentAuthMethod.ps1`'s `AUTH-…` violation persist payload (`UnauthorizedServiceAccount` / legacy-client-secret detections) — added `fsi_detectedat = (Get-Date).ToUniversalTime().ToString('o')` there too so AUTH violations also appear in auditor evidence.
- **Schema description accuracy — `fsi_approvedscopes`/`fsi_actualscopes` on `fsi_credentialviolation` documented as "JSON array" but written as space-separated tokens:** The column descriptions added in an earlier release said "JSON array of approved OAuth scopes" / "JSON array of actual OAuth scopes observed", but the v2.1.2 persist path writes the data as a space-separated token list (matches the format on the sibling `fsi_agentconnectorscope` columns). Updated both column descriptions to accurately reflect the persisted shape. Pure schema-metadata change; no runtime impact and no migration required (existing rows continue to work).
- **Evidence completeness — `Export-CredentialEvidence.ps1` `$violationSelect` did not include scope columns:** The exported JSON had no `fsi_approvedscopes` / `fsi_actualscopes` fields on each violation, so auditors received only the recommendation prose and could not independently verify the scope diff. Widened `$violationSelect` to include both columns. Mirrors the schema-completeness intent of the v2.1.1 `[Unreleased]` columns.
...
- Lab-readiness static validation pass (no live tenant). All 9 PowerShell scripts parse cleanly; all 3 Python scripts compile; column references cross-checked against `create_cod_dataverse_schema.py`; auth token audiences and current Microsoft APIs verified against Microsoft Learn.
- **Live tenant validation (FSI-AgentGov-Lab on autojude, 2026-06-15) — COMPLETE end-to-end.** All 7 stages executed against `https://autojude.crm.dynamics.com/`: Stage 1 schema (5 tables, ~80 columns, 6 option sets incl. shared `fsi_acv_zone` reused at `100000000`-based integers, 1 relationship); Stage 2 conn refs (4); Stage 3 env vars (11); Stage 5 single-flight on a synthetic `fsi_agentconnectorscope` row → `Compare-OAuthScopeBaseline.ps1` detected 4 excess (all sensitive → Critical) + 1 missing, persisted a SCOPE-… violation; Stage 6 `Export-CredentialEvidence.ps1` captured 1 critical violation, SHA-256 `D51A8F751663AE40CC4B4F2DEECEAD863C12CDC45735AF07A8B355CAF4D5FA94` verified 3 ways (`Test-EvidenceIntegrity.ps1` exit 0, `Get-FileHash` recompute, `.sha256` companion); Stage 7 teardown clean (all 5 tables back to 0 rows). The five live-deploy bug fixes in this changelog all surfaced and were patched during this pass. **No real Copilot Studio agents were touched.** Lab-side evidence in [FSI-AgentGov-Lab `credential-oversharing-detector/LAB-VALIDATION.md` §7](https://github.com/judep_microsoft/FSI-AgentGov-Lab/blob/main/credential-oversharing-detector/LAB-VALIDATION.md).
