...
<#
.SYNOPSIS
    Verifies SHA-256 integrity of credential oversharing evidence files.
...
.DESCRIPTION
    ...
    Evidence files produced by Export-CredentialEvidence.ps1 include SHA-256
...
.EXAMPLE
    .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath ".\evidence\cod-evidence-All-20260209-143022.json"
...
.EXAMPLE
    Get-ChildItem .\evidence\cod-evidence-*.json | ForEach-Object {
        .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath $_.FullName
    }

    Batch verification of all COD evidence files in a directory. Iterates through
    all matching JSON files and verifies each one against its companion hash file.

.EXAMPLE
    $isValid = .\Test-EvidenceIntegrity.ps1 `
        -EvidenceFilePath ".\evidence\cod-evidence-All-20260209-143022.json" `
        -Quiet
...
.NOTES
    ...
    - Example: "abc123...  cod-evidence-All-20260209-143022.json"
