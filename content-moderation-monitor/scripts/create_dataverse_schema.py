...
BASELINE_COLUMNS = [
    _string_col("fsi_environment_guid", "Environment GUID", 100,
                description="Power Platform environment GUID"),
    _string_col("fsi_environment_name", "Environment Name", 500,
                description="Environment display name"),
    _picklist_col("fsi_zone", "Zone", "fsi_acv_zone",
                  description="Zone classification"),
    _string_col("fsi_agent_id", "Agent ID", 100,
                description="Copilot Studio bot GUID"),
    _string_col("fsi_agent_name", "Agent Name", 500,
                description="Agent display name"),
    _string_col("fsi_moderation_level", "Moderation Level", 50,
    ...
    _datetime_col("fsi_captured_at", "Captured At",
                  description="When baseline was captured"),
    _string_col("fsi_captured_by", "Captured By", 200, required=False,
                description="UPN of capturing operator"),
    _memo_col("fsi_raw_json", "Raw JSON", 100000,
              description="Full JSON snapshot of moderation settings"),
]

HISTORY_COLUMNS = [
    _string_col("fsi_run_id", "Run ID", 36,
                description="GUID correlating all records in one scan run"),
    _datetime_col("fsi_validation_time", "Validation Time",
                  description="When scan executed"),
    _integer_col("fsi_total_agents", "Total Agents",
                 description="Total agents scanned"),
    _integer_col("fsi_compliant_count", "Compliant Count",
                 description="Agents passing moderation checks"),
    _integer_col("fsi_violation_count", "Violation Count",
                 description="Agents with moderation violations"),
    _string_col("fsi_overall_status", "Overall Status", 50,
                description="Passed/Failed/Warning/Critical"),
    _string_col("fsi_environments_scanned", "Environments Scanned", 2000,
                required=False,
                description="Comma-separated environments covered"),
    _memo_col("fsi_summary_json", "Summary JSON", 100000,
...
VIOLATION_COLUMNS = [
    ...
    _picklist_col("fsi_zone", "Zone", "fsi_acv_zone",
                  description="Zone classification"),
    _string_col("fsi_expected_level", "Expected Level", 50,
                description="Zone-required moderation level"),
    _string_col("fsi_actual_level", "Actual Level", 50,
                description="Agent's current moderation level"),
    _picklist_col("fsi_severity", "Severity", "fsi_acv_severity",
                  description="Violation severity"),
    _string_col("fsi_regulatory_context", "Regulatory Context", 2000,
                required=False,
                description="FINRA/SOX/GLBA regulatory impact context"),
    _datetime_col("fsi_detected_at", "Detected At",
                  description="When violation was detected"),
    _string_col("fsi_run_id", "Run ID", 36, required=False,
