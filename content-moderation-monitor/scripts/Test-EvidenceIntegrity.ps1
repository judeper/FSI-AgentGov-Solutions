...
<#
.SYNOPSIS
    Verifies SHA-256 integrity of content moderation evidence files.
...
.DESCRIPTION
    ...
    Evidence files produced by Export-ContentModerationEvidence include SHA-256
...
.EXAMPLE
    .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath ".\evidence\cmm-evidence-All-20260210-143022.json"
...
.EXAMPLE
    Get-ChildItem .\evidence\cmm-evidence-*.json | ForEach-Object {
        .\Test-EvidenceIntegrity.ps1 -EvidenceFilePath $_.FullName
    }

    Batch verification of all CMM evidence files in a directory. Iterates through
    all matching JSON files and verifies each one against its companion hash file.

.EXAMPLE
    $isValid = .\Test-EvidenceIntegrity.ps1 `
        -EvidenceFilePath ".\evidence\cmm-evidence-All-20260210-143022.json" `
        -Quiet
...
.NOTES
    ...
    - Example: "abc123...  cmm-evidence-All-20260210-143022.json"
