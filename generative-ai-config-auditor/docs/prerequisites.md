...
| `*.openai.azure.com` | HTTPS | AOAI endpoint validation (connectivity check only) |
...
| Table | Purpose |
|-------|---------|
| `fsi_GACBaseline` | Per-agent generative AI configuration snapshots |
| `fsi_GACValidationHistory` | Immutable scan summary records |
| `fsi_GACViolation` | Per-agent violations with severity |
| `fsi_GACApprovedConnection` | Approved Azure OpenAI connections |
| `fsi_GACFeatureInventory` | Per-agent feature tracking |
