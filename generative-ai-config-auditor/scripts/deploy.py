#!/usr/bin/env python3
"""Deploy Generative AI Config Auditor infrastructure to Dataverse.

Orchestrates the full deployment pipeline:
  1. Test Connection — validate Dataverse API access
  2. Dataverse Schema — tables, columns, shared and GAC option sets
...
2. CONNECTIONS — Bind connection references in Power Automate:
   ...
   - fsi_cr_office365_genaiconfigauditor       → Select Office 365 connection
   - fsi_cr_teams_genaiconfigauditor           → Select Teams connection
   - fsi_cr_azureautomation_genaiconfigauditor → Select Azure Automation connection

3. APPROVED CONNECTIONS — Populate fsi_GACApprovedConnection table:
   - Add approved Azure OpenAI connections per zone
   - Set fsi_AoaiWhitelistMode environment variable to 'enforced'
...
def run_deployment(
    ...
    """Execute the GAC deployment pipeline.
    ...
    print()
    print("=" * 60)
    print("Generative AI Config Auditor — Dataverse Deployment")
    print("=" * 60)
...
def main() -> None:
    """CLI entry point for GAC deployment orchestrator."""
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  GAC_TENANT_ID        Microsoft Entra ID tenant ID\n"
            "  GAC_CLIENT_ID        Service principal app ID\n"
            "  GAC_CLIENT_SECRET    Service principal secret\n"
            "  GAC_ENVIRONMENT_URL  Dataverse environment URL\n"
        ),
    )
