<#
.SYNOPSIS
    Validates generative AI configuration for all Copilot Studio agents against
    zone-specific governance requirements.

.DESCRIPTION
    Orchestrates a full generative AI configuration compliance scan:
    1. Enumerates Power Platform environments
    2. Queries each environment's Dataverse for Copilot Studio agents
    3. Retrieves generative AI settings per agent (AOAI, orchestration, gen answers)
    4. Validates against zone policies (AOAI allowed, orchestration mode, gen answers)
    5. Reports violations with severity classification and regulatory context

    This is the primary validation script for the Generative AI Config Auditor
    (GAC) solution. It validates per-agent generative AI feature enablement
    against zone-based governance policies defined by Control 2.24.

    Combines Get-AgentGenAISettings and Compare-GenAIConfigCompliance into
    a single validation workflow with dry-run mode, multiple output formats,
    summary statistics, and environment/agent filtering.

.NOTES
    File: Test-GenAIConfigCompliance.ps1
    Version: 1.0.0
...
function Test-GenAIConfigCompliance {
    ...
    .DESCRIPTION
        ...
        1. Enumerates Power Platform environments
        2. Queries each environment's Dataverse for Copilot Studio agents
        3. Retrieves generative AI settings per agent
        4. Validates against zone policies
        5. Reports violations with severity classification and regulatory context
    ...
    .PARAMETER PersistResults
        ...
        fsi_gacvalidationhistory and individual violations to
        fsi_gacviolations. Requires active Dataverse connection.
    ...
    .EXAMPLE
        ...
        Test-GenAIConfigCompliance -WhatIf

        Dry-run scan of all environments (default: published agents, violations only).

    .EXAMPLE
        . ./Test-GenAIConfigCompliance.ps1
        Test-GenAIConfigCompliance -IncludeEnvironments @("env-id-1", "env-id-2") -OutputFormat Json

        Scan specific environments with JSON output for evidence pipeline.

    .EXAMPLE
        . ./Test-GenAIConfigCompliance.ps1
        Test-GenAIConfigCompliance -IncludeCompliant -IncludeDrafts

        Full scan including compliant agents and drafts.

    .EXAMPLE
        . ./Test-GenAIConfigCompliance.ps1
        Test-GenAIConfigCompliance -ExcludeSandbox -ExcludeTrial -DataverseUrl "https://org.crm.dynamics.com"
    ...
    Write-Verbose "========================================="
    Write-Verbose "Generative AI Config Auditor v1.0.0"
    ...
    $compareComplianceScript = Join-Path $scriptRoot 'Compare-GenAIConfigCompliance.ps1'
    ...
    Write-Verbose "Loading Get-AgentGenAISettings from: $getSettingsScript"
    . $getSettingsScript

    Write-Verbose "Loading Compare-GenAIConfigCompliance from: $compareComplianceScript"
    ...
    if ($DataverseUrl) {
        try {
            ...
            if ($DataverseToken) {
                ...
                Connect-GACDataverse -DataverseUrl $DataverseUrl
            }

            $connection = Get-GACConnection
            if ($connection.IsConnected) {
                ...
                $dvGracePeriod = Get-GACEnvironmentVariable -Name 'GracePeriodHours' -DefaultValue $null
                ...
                $dvIncludeDrafts = Get-GACEnvironmentVariable -Name 'IncludeDrafts' -DefaultValue $null
                ...
                $dvIncludeSandbox = Get-GACEnvironmentVariable -Name 'IncludeSandbox' -DefaultValue $null
    ...
    # Build parameter hashtable for Get-AgentGenAISettings
    $queryParams = @{
    ...
    # Build parameter hashtable for Compare-GenAIConfigCompliance
    $compareParams = @{}
    ...
    if ($PSCmdlet.ShouldProcess("All Power Platform environments", "Query agent generative AI settings")) {
    ...
    Write-Verbose "Comparing agent GenAI settings against zone policies..."

    $complianceResults = $agentSettings | Compare-GenAIConfigCompliance @compareParams
    ...
    Write-Host "  Generative AI Config Compliance Scan" -ForegroundColor Cyan
    Write-Host "  Control 2.24 - Feature Enablement Governance" -ForegroundColor Gray
    ...
    if ($PersistResults -and $dataverseConnected) {
        ...
        if ($PSCmdlet.ShouldProcess("Dataverse validation history", "Write scan results")) {
            try {
                Write-GACValidationHistory -ValidationResult $validationSummary -RunId $runId
        ...
        'Table' {
            if ($outputResults.Count -gt 0) {
                ...
                foreach ($agent in $outputResults) {
                    ...
                    Write-Host "    AOAI Enabled:     $($agent.AzureOpenAIEnabled)" -ForegroundColor Gray
                    Write-Host "    Orchestration:    $($agent.OrchestrationMode)" -ForegroundColor Gray
                    Write-Host "    Gen Answers:      $($agent.GenerativeAnswersNodeCount) node(s)" -ForegroundColor Gray
                    Write-Host "    Status:           $($agent.AgentStatus)" -ForegroundColor Gray
                    if (-not $agent.IsCompliant) {
                        Write-Host "    Violation:        $($agent.ViolationType)" -ForegroundColor $severityColor
