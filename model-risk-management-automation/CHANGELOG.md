...
- **Minor (docs)**: `README.md` *Prerequisites → Roles* and `docs/prerequisites.md` (roles table + *Granting Permissions* step 4) stated that **Application Administrator** could grant the managed identity's API permissions. This solution requests Microsoft Graph *application* permissions (`User.Read.All`, `Sites.ReadWrite.All`), and Microsoft Learn documents that Cloud Application Administrator, AI Administrator, and Application Administrator can consent to any API **except Microsoft Graph app roles (application permissions)**. Corrected to **Global Administrator or Privileged Role Administrator**, with the Application Administrator limitation noted. `README.md:129`, `docs/prerequisites.md:23,58`. Ref: https://learn.microsoft.com/entra/identity/enterprise-apps/grant-admin-consent (drift re-verification vs Microsoft Learn, issue #360)
- **Minor (docs)**: `README.md` *Agent ID Migration Evidence* sample JSON and CSV represented an Entra Agent ID as a URN string (`urn:ms:agent:contoso:customer-service-assistant`). Microsoft Learn documents the Entra Agent ID as a **GUID**, retrievable in Copilot Studio under **Settings** → **Advanced** → **Metadata** → **Entra Agent ID**. Replaced the URN sample values with GUIDs in both the JSON evidence record and the examiner-facing CSV export. `README.md:281,297-299`. Ref: https://learn.microsoft.com/microsoft-copilot-studio/admin-use-entra-agent-identities (drift re-verification vs Microsoft Learn, issue #360)
- **Minor (docs)**: `README.md` *Prerequisites → Dependencies* described the `agent-365-lifecycle-governance` cross-reference as available only "when preview registry APIs are available". The Agent 365 agent registry Microsoft Graph APIs are now published in preview (List packages / Get Copilot package details) and require the AI Administrator or Global Administrator role. Updated the dependency note to state their current preview availability and role requirement. `README.md:112`. Refs: https://learn.microsoft.com/microsoft-agent-365/admin/graph-api and https://learn.microsoft.com/entra/agent-id/agent-registry-convergence (drift re-verification vs Microsoft Learn, issue #360)
...
- **Docs**: `README.md` *Agent ID Migration Evidence → Background* now records the verified Entra Agent ID rollout behaviour: Copilot Studio automatically creates a Microsoft Entra Agent ID for each new agent created after the July 2026 rollout; agents created before the rollout continue to use app registrations and are scheduled for migration by Microsoft; and governance capabilities work for both Agent IDs and App Registration IDs during the transition period. Also documents the Copilot Studio portal path used to retrieve the Agent ID GUID. Ref: https://learn.microsoft.com/microsoft-copilot-studio/admin-use-entra-agent-identities

### Changed

- **Docs**: Refreshed the `README.md` `Last Verified` marker to 2026-07-26 following a full re-verification of the file's Microsoft product claims against current Microsoft Learn documentation. Claims re-checked and confirmed unchanged: AI-powered **self-healing for Power Automate desktop flows remains a preview feature**, is powered by GPT-4.1 mini and Claude Sonnet 4.5 in combination, applies only to *Element not found* errors on specific single-element UI and browser automation actions, and is runtime-only (no effect during designer runs); **Dataverse long-term retention still requires a Managed Environment**; the **Word Online (Business)** connector remains published for Power Automate; and **Power Platform Admin** plus Dataverse **System Administrator** remain valid role names for environment enumeration, `bot` table access, and table creation. No regulatory or compliance wording was reviewed, added, or altered. Refs: https://learn.microsoft.com/power-automate/desktop-flows/self-healing, https://learn.microsoft.com/power-apps/maker/data-platform/data-retention-overview
